    ...
    <div className="flex min-h-0 flex-col gap-y-8 w-full">
        ...
        <Timeline>
          {hackathons.map((hackathon) => {
            const year = extractYear(hackathon.dates);
            return (
            <TimelineItem key={hackathon.title + hackathon.dates} className="w-full flex items-start justify-between gap-10">
              ...
              <div className="flex flex-1 flex-col justify-start gap-2 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {year && (
                    <span className="text-[10px] font-medium bg-muted text-muted-foreground px-1.5 py-0.5 rounded">{year}</span>
                  )}
                </div>
                {hackathon.title && (
                ...
                {hackathon.description && (
                  <div className="prose max-w-full text-sm text-muted-foreground leading-relaxed dark:prose-invert">
                    <Markdown rehypePlugins={[rehypeRaw]}>{hackathon.description}</Markdown>
                  </div>
                )}
